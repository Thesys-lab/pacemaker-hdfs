...
class FSDirWriteFileOp {
  static final Logger LOG = LoggerFactory.getLogger(FSDirWriteFileOp.class);
  ...
  static DatanodeStorageInfo[] chooseTargetForNewBlock(
      ...
      ValidateAddBlockResult r, String diskGroup) throws IOException {
    ...
    if (!ignoreClientLocality) {
      LOG.info("Disk-group chosen for allocation of first block of file: " + src + " = " + diskGroup);
      clientNode = getClientNode(bm, r.clientMachine, diskGroup);
    } else {
      LOG.info("Disk-group chosen for allocation of subsequent blocks of file: " + src + " = " + diskGroup);
    }
    ...
    return bm.chooseTarget4NewBlock(src, r.numTargets, clientNode,
        ...
        r.blockType, r.ecPolicy, flags, diskGroup);
  }
   ...
   * Resolve clientmachine address to get a network location path
   */
  static Node getClientNode(BlockManager bm, String clientMachine, String diskGroup) {
    List<String> hosts = new ArrayList<>(1);
    hosts.add(clientMachine);

    // HeART enabled HDFS should choose a disk-group at this point to return the first client.
    List<String> rName = bm.getDatanodeManager(diskGroup)
        .resolveNetworkLocation(hosts);
    Node clientNode = null;
    if (rName != null) {
      // Able to resolve clientMachine mapping.
  ...
  static HdfsFileStatus startFile(
      ...
      boolean shouldReplicate, String ecPolicyName, boolean logRetryEntry, String diskGroup)
      throws IOException {
  ...
  private static INodesInPath addFile(
      ...
      String ecPolicyName, String diskGroup) throws IOException {
    ...
    try {
      ...
      INodeFile newNode = newINodeFile(fsd.allocateNewInodeId(), permissions,
          ...
          blockType, diskGroup);
      newNode.setLocalName(localName);
      newNode.toUnderConstruction(clientName, clientMachine, diskGroup);
  ...
  private static INodeFile newINodeFile(
      ...
      byte storagePolicyId, BlockType blockType, String diskGroup) {
    return new INodeFile(id, null, permissions, mtime, atime,
        BlockInfo.EMPTY_ARRAY, replication, ecPolicyID, preferredBlockSize,
        storagePolicyId, blockType, diskGroup);
  }
  ...
  private static INodeFile newINodeFile(long id, PermissionStatus permissions,
                                        ...
                                        long preferredBlockSize, BlockType blockType, String diskGroup) {
    return newINodeFile(id, permissions, mtime, atime, replication, ecPolicyID,
        preferredBlockSize, (byte)0, blockType, diskGroup);
  }
