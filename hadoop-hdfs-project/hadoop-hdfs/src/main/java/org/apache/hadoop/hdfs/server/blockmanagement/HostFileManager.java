...
public class HostFileManager extends HostConfigManager {
  ...
  public void refresh(String diskGroupHosts) throws IOException {
    refresh(conf.get(DFSConfigKeys.DFS_HOSTS, "/tmp/hadoop-3.2.0/etc/hadoop/" + diskGroupHosts + ".txt"),
        conf.get(DFSConfigKeys.DFS_HOSTS_EXCLUDE, "/tmp/hadoop-3.2.0/etc/hadoop/" + diskGroupHosts + "-excludes.txt"));
  }
