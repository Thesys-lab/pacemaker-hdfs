...
public class DatanodeManager {
  ...
  private HashMap<String, String> datanode2DiskGroupMap = null;
  ...
  DatanodeManager(final BlockManager blockManager, final Namesystem namesystem,
      final Configuration conf, final String diskGroup,
                  HashMap<String, String> datanode2DiskGroupMap) throws IOException {
    this.namesystem = namesystem;
    this.blockManager = blockManager;
    this.diskGroup = diskGroup;
    this.datanode2DiskGroupMap = datanode2DiskGroupMap;
    ...
    try {
      if(!diskGroup.equals("")) {
        this.hostConfigManager.setDiskGroup(diskGroup);
        this.hostConfigManager.refresh(diskGroup);
      } else {
        this.hostConfigManager.refresh();
      }
    } catch (IOException e) {
  ...
  public void registerDatanode(DatanodeRegistration nodeReg)
    ...
    try {
      ...
      NameNode.stateChangeLog.info("BLOCK* registerDatanode: from "
          + nodeReg + " storage " + nodeReg.getDatanodeUuid() + " to disk-group " + diskGroup);
  ...
  private DatanodeID parseDNFromHostsEntry(String hostLine) {
    ...
    if (datanode2DiskGroupMap != null) {
      LOG.info("Adding host string " + hostStr + " to disk group " + diskGroup);
      datanode2DiskGroupMap.put(diskGroup, hostStr);
    }
    ...
    if (listDeadNodes) {
      for (InetSocketAddress addr : includedNodes) {
        ...
        DatanodeDescriptor dn = new DatanodeDescriptor(new DatanodeID(addr
                ...
                defaultInfoPort, defaultInfoSecurePort, defaultIpcPort, diskGroup));
        setDatanodeDead(dn);
