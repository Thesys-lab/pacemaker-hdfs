...
public class BlockManager implements BlockStatsMXBean {
  ...
  private final HashMap<String, DatanodeManager> diskGroup2DatanodeManagerMap;
  private final HeartbeatManager heartbeatManager;
  private final HashMap<String, HeartbeatManager> diskGroup2HeartbeatManagerMap;
  private final BlockTokenSecretManager blockTokenSecretManager;

  private final HashMap<String, String> datanode2DiskGroupMap;
  ...
  private final HashMap<String, BlockPlacementPolicies> diskGroup2PlacementPoliciesMap;
  ...
  private Random diskGroupRandomizer;

  public String chooseDiskGroupRandomly() {
    String selectedDiskGroup = diskGroup2DatanodeManagerMap.keySet().toArray()[
        diskGroupRandomizer.nextInt(diskGroup2DatanodeManagerMap.keySet().size())].toString();
    LOG.info("Selected disk-group: " + selectedDiskGroup);
    return selectedDiskGroup;
  }

  public HashMap<String, String> getDatanode2DiskGroupMap() {
    return datanode2DiskGroupMap;
  }

  public BlockManager(final Namesystem namesystem, boolean haEnabled,
      final Configuration conf) throws IOException {
    this.namesystem = namesystem;
    diskGroupRandomizer = new Random();
    diskGroup2DatanodeManagerMap = new HashMap<>();
    diskGroup2PlacementPoliciesMap = new HashMap<>();
    diskGroup2HeartbeatManagerMap = new HashMap<>();
    datanode2DiskGroupMap = new HashMap<>();
    Collection<String> allRedundancySchemes = conf.getStringCollection(DFSConfigKeys.HEART_DISK_GROUPS_LIST);
    for (String redundancyScheme : allRedundancySchemes) {
      DatanodeManager dnode = new DatanodeManager(
          this, namesystem, conf, redundancyScheme, datanode2DiskGroupMap);
      //FIXME: HeART: revert the constructor signature of DatanodeManager.
      LOG.info("Datanode manager for disk-group" + redundancyScheme + " = " + dnode.toString());
      diskGroup2DatanodeManagerMap.put(redundancyScheme, dnode);
      diskGroup2PlacementPoliciesMap.put(redundancyScheme, new BlockPlacementPolicies(
          conf, dnode.getFSClusterStats(), dnode.getNetworkTopology(), dnode.getHost2DatanodeMap()));
      diskGroup2HeartbeatManagerMap.put(redundancyScheme, dnode.getHeartbeatManager());
      Iterator itIncludedNodes = dnode.getHostConfigManager().getIncludes().iterator();
      while(itIncludedNodes.hasNext()) {
        datanode2DiskGroupMap.put(itIncludedNodes.next().toString().split("/", 2)[0], redundancyScheme);
      }
    }

    LOG.info("Datanode2DiskGroupMap = " + datanode2DiskGroupMap.toString());

    datanodeManager = new DatanodeManager(this, namesystem, conf);
    heartbeatManager = datanodeManager.getHeartbeatManager();
    this.blockIdManager = new BlockIdManager(this);

    //FIXME: HeART: need to fix the rescan stuff. Might be more relevant when we cause failures.
  ...
  public void activate(Configuration conf, long blockTotal) {
    ...
    Collection<String> allRedundancySchemes = conf.getStringCollection(DFSConfigKeys.HEART_DISK_GROUPS_LIST);
    for (String redundancyScheme : allRedundancySchemes) {
      diskGroup2DatanodeManagerMap.get(redundancyScheme).activate(conf);
    }
  ...
  public void close() {
    ...
    for (String redundancyScheme : diskGroup2DatanodeManagerMap.keySet()) {
      diskGroup2DatanodeManagerMap.get(redundancyScheme).close();
    }
  ...
  /** @return the datanodeManager for a disk-group */
  public DatanodeManager getDatanodeManager(String diskGroup) {
    return diskGroup2DatanodeManagerMap.get(diskGroup);
  }
  ...
  //FIXME: HeART: not handled since this function is only about printing metadata on the screen.
   ...
   */
  //FIXME: HeART not handled this case because it appears only in append, and that's not supported in EC HDFS.
  ...
  public BlocksWithLocations getBlocksWithLocations(final DatanodeID datanode,
    ...
    final DatanodeDescriptor node = getDatanodeManager(
        datanode2DiskGroupMap.get(datanode.getHostName())).getDatanode(datanode);
    if (node == null) {
  ...
  public void findAndMarkBlockAsCorrupt(final ExtendedBlock blk,
    ...
    DatanodeDescriptor node = getDatanodeManager(datanode2DiskGroupMap.get(dn.getHostName())).getDatanode(dn);
    if (node == null) {
...
//      String diskGroup = ((DatanodeID) rw.getBlock().getDatanode(0)).getDiskGroup();
//      LOG.info("HeART: Disk group returned by getDiskGroup = " + diskGroup);
      String diskGroup = "";
      Iterator<DatanodeStorageInfo> itStorageInfos = rw.getBlock().getStorageInfos();
      while(itStorageInfos.hasNext()) {
        DatanodeStorageInfo dsi = itStorageInfos.next();
        if (dsi == null) {
          continue;
        }
        diskGroup = datanode2DiskGroupMap.get(((DatanodeID) dsi.getDatanodeDescriptor()).getHostName());
        break;
      }
//      LOG.info("HeART: Disk group returned by getHostName = " + diskGroup);
      final BlockPlacementPolicy placementPolicy = diskGroup2PlacementPoliciesMap.get(diskGroup).getPolicy(
          rw.getBlock().getBlockType());
...
//          placementPolicies.getPolicy(rw.getBlock().getBlockType());
  ...
  public DatanodeStorageInfo[] chooseTarget4NewBlock(final String src,
    ...
    final BlockPlacementPolicy blockplacement =
        placementPolicies.getPolicy(blockType);
    LOG.info("In chooseTarget4NewBlock of BlockManager, placement policy = " + blockplacement.getClass().getName());
   ...
   *           if the number of targets < minimum replication.
   * @see BlockPlacementPolicy#chooseTarget(String, int, Node,
   *      Set, long, List, BlockStoragePolicy, EnumSet, String)
   */
  public DatanodeStorageInfo[] chooseTarget4NewBlock(final String src,
                                                     final int numOfReplicas, final Node client,
                                                     final Set<Node> excludedNodes,
                                                     ...
                                                     String diskGroup) throws IOException {
    List<DatanodeDescriptor> favoredDatanodeDescriptors =
        getDatanodeDescriptors(favoredNodes, diskGroup);
    final BlockStoragePolicy storagePolicy =
        storagePolicySuite.getPolicy(storagePolicyID);
//    final BlockPlacementPolicy blockplacement =
//        placementPolicies.getPolicy(blockType);
    final BlockPlacementPolicy blockplacement =
        (diskGroup2PlacementPoliciesMap.get(diskGroup)).getPolicy(blockType);
    LOG.info("In HeART based chooseTarget4NewBlock, placement policy = " + blockplacement.getClass().getName());
    final DatanodeStorageInfo[] targets = blockplacement.chooseTarget(src,
        numOfReplicas, client, excludedNodes, blocksize,
        favoredDatanodeDescriptors, storagePolicy, flags);
    ...
    if (blockType == BlockType.CONTIGUOUS && targets.length < minReplication) {
      throw new IOException(String.format(errorMessage, src,
          targets.length, minReplication, "minReplication nodes",
          getDatanodeManager(diskGroup).getNetworkTopology().getNumOfLeaves(),
          (excludedNodes == null? "no": excludedNodes.size())));
    } else if (blockType == BlockType.STRIPED &&
        targets.length < ecPolicy.getNumDataUnits()) {
      throw new IOException(
   ...
   * hostaddress:port or just hostaddress.
   */
  List<DatanodeDescriptor> getDatanodeDescriptors(List<String> nodes, String diskGroup) {
    List<DatanodeDescriptor> datanodeDescriptors = null;
    if (nodes != null) {
      datanodeDescriptors = new ArrayList<DatanodeDescriptor>(nodes.size());
      for (int i = 0; i < nodes.size(); i++) {
        DatanodeDescriptor node = diskGroup2DatanodeManagerMap.get(diskGroup).getDatanodeDescriptor(nodes.get(i));
        if (node != null) {
          datanodeDescriptors.add(node);
        }
      }
  ...
  public long requestBlockReportLeaseId(DatanodeRegistration nodeReg) {
    ...
    try {
      node = diskGroup2DatanodeManagerMap.get(datanode2DiskGroupMap.get(nodeReg.getHostName())).getDatanode(nodeReg);
  ...
  public void registerDatanode(DatanodeRegistration nodeReg)
      throws IOException {
    assert namesystem.hasWriteLock();
    LOG.info("Searching for node " + nodeReg.getHostName());
    LOG.info("Current disk-group is " + datanode2DiskGroupMap.get(nodeReg.getHostName()));
    // check if the correct datanodemanager contains the registering datanode.
    DatanodeManager oldDatanodeMgr = diskGroup2DatanodeManagerMap.get(datanode2DiskGroupMap.get(nodeReg.getHostName()));
    Iterator itIncludedNodes = oldDatanodeMgr.getHostConfigManager().getIncludes().iterator();
    boolean doesDatanodeExist = false;
    while(itIncludedNodes.hasNext()) {
      if(itIncludedNodes.next().toString().split("/", 2)[0].equals(nodeReg.getHostName())) {
        doesDatanodeExist = true;
        break;
      }
    }

    if(!doesDatanodeExist) {
      DatanodeManager newDatanodeMgr = null;
      String newDiskGroup = null;
      Iterator<Map.Entry<String, DatanodeManager>> itDatanodeMgr = diskGroup2DatanodeManagerMap.entrySet().iterator();
      while(itDatanodeMgr.hasNext()) {
        Map.Entry<String, DatanodeManager> pair = itDatanodeMgr.next();
        newDatanodeMgr = pair.getValue();
        newDiskGroup = pair.getKey();
        if(newDatanodeMgr.equals(oldDatanodeMgr)) {
          continue;
        }
        itIncludedNodes = newDatanodeMgr.getHostConfigManager().getIncludes().iterator();
        while(itIncludedNodes.hasNext()) {
          if(itIncludedNodes.next().toString().split("/", 2)[0].equals(nodeReg.getHostName())) {
            doesDatanodeExist = true;
            break;
        ...
        if(doesDatanodeExist) {
          LOG.info("New disk-group is " + newDiskGroup);
          newDatanodeMgr.registerDatanode(nodeReg);
          datanode2DiskGroupMap.remove(nodeReg.getHostName());
          datanode2DiskGroupMap.put(nodeReg.getHostName(), newDiskGroup);
          break;
        }
      }
    } else {
      diskGroup2DatanodeManagerMap.get(datanode2DiskGroupMap.get(nodeReg.getHostName())).registerDatanode(nodeReg);
    }

    //FIXME: HeART: not fixed because it mostly pertains to datanodes added on the fly.
//    datanodeManager.registerDatanode(nodeReg);
  ...
  public boolean processReport(final DatanodeID nodeID,
    ...
    try {
      node = diskGroup2DatanodeManagerMap.get(datanode2DiskGroupMap.get(nodeID.getHostName())).getDatanode(nodeID);
...
//    BlockPlacementPolicy placementPolicy = placementPolicies.getPolicy(STRIPED);
    ...
    String diskGroup = datanode2DiskGroupMap.get(((DatanodeID) storedBlock.getDatanode(0)).getHostName());
    LOG.info("HeART: Disk group chosen to remove extra replicas = " + diskGroup);
    BlockPlacementPolicy placementPolicy = diskGroup2PlacementPoliciesMap.get(diskGroup).getPolicy(storedBlock.getBlockType());
  ...
  public void addBlock(DatanodeStorageInfo storageInfo, Block block,
    ...
    if (delHint != null && delHint.length() != 0) {
      delHintNode = diskGroup2DatanodeManagerMap.get(
          datanode2DiskGroupMap.get(node.getHostName())).getDatanode(delHint);
      if (delHintNode == null) {
  ...
  public void processIncrementalBlockReport(final DatanodeID nodeID,
    ...
    assert namesystem.hasWriteLock();
    final DatanodeDescriptor node = diskGroup2DatanodeManagerMap.get(
        datanode2DiskGroupMap.get(nodeID.getHostName())).getDatanode(nodeID);
    if (node == null || !node.isRegistered()) {
  ...
  private int invalidateWorkForOneNode(DatanodeInfo dn) {
    ...
    try {
      ...
      try {
        DatanodeDescriptor dnDescriptor = diskGroup2DatanodeManagerMap.get(
            datanode2DiskGroupMap.get(dn.getHostName())).getDatanode(dn);
 ...
 public String getCorruptReason(Block block, DatanodeDescriptor node) {
  ...
  private class StorageInfoDefragmenter implements Runnable {
    ...
    private void scanAndCompactStorages() throws InterruptedException {
      ...
      Iterator<String> itDg = diskGroup2DatanodeManagerMap.keySet().iterator();
      while (itDg.hasNext()) {
        String diskGroup = itDg.next();
        for (DatanodeDescriptor node
            : diskGroup2DatanodeManagerMap.get(diskGroup).getDatanodeListForReport(DatanodeReportType.ALL)) {
          for (DatanodeStorageInfo storage : node.getStorageInfos()) {
            try {
              namesystem.readLock();
              double ratio = storage.treeSetFillRatio();
              if (ratio < storageInfoDefragmentRatio) {
                datanodesAndStorages.add(diskGroup);
      ...
      if (!datanodesAndStorages.isEmpty()) {
        for (int i = 0; i < datanodesAndStorages.size(); i += 3) {
          namesystem.writeLock();
          try {
            final DatanodeDescriptor dn = diskGroup2DatanodeManagerMap.get(
                datanodesAndStorages.get(i)).getDatanode(datanodesAndStorages.get(i + 1));
            if (dn == null) {
              continue;
            }
            final DatanodeStorageInfo storage = dn.getStorageInfo(datanodesAndStorages.get(i + 2));
            if (storage != null) {
              boolean aborted =
...
//
//    LOG.info("HeART: BEFORE numLive = " + numlive);

    int numlive = 0;
    Iterator itHbMgr = diskGroup2HeartbeatManagerMap.keySet().iterator();
    while(itHbMgr.hasNext()) {
      numlive += diskGroup2HeartbeatManagerMap.get(itHbMgr.next()).getLiveDatanodeCount();
    }
...
//    LOG.info("HeART: AFTER numLive = " + numlive + " and workFound = " + workFound);
  ...
  public void clearQueues() {
    ...
    for (DatanodeManager dm : diskGroup2DatanodeManagerMap.values()) {
      dm.clearPendingQueues();
    }
  ...
  //FIXME: HeART: not handled because it's only used in tests.
